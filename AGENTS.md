...
**Current lab (until the operator says otherwise):** **AWS colocated** — ktranslate **and** the Alloy network fork in parallel. Alloy SNMP scrape tiers are **hot + cold + topology** (`LAB_ALLOY_SNMP_TOPOLOGY=1`, 2m lab interval; Nokia `modules_topology` is pinned via `snmp-overrides.yml` because the fingerprinter still emits `[]`). Do **not** start the laptop Clos. Learners on **networko11ydev** get ktranslate (`GC_OTLP_*_2`). The workshop stack (`befd61marcnetterfield879`) is `GC_OTLP_*_3`. Restore: `python3 local/scripts/ssm-alloy-ktranslate-parallel.py` (or on-host `make -C local alloy-ktranslate-parallel-colocated`). Full Alloy cutover remains `make -C local alloy-cutover-colocated` (`LAB_KTRANSLATE=0`).

**SE customer demo (networko11ydev, ktranslate only):** [`docs/ktranslate-se-demo-talk-track.md`](docs/ktranslate-se-demo-talk-track.md) — click path + SolarWinds “what they just asked” Q&A. Do not send SEs to the Alloy Network Fork folder.

**Workshop webinar (marcnetterfield1 / `workshop-ktranslate`):** student SNMP story is the **colocated 3-site Clos** — `srl-hq` (`spine1`, `leaf1`, `leaf2`), `srl-branch1` (`leaf-br1`), `srl-branch2` (`leaf-br2`). Restore: `python3 local/scripts/ssm-alloy-ktranslate-parallel.py`. Student order: login → data sources → synthetics → import dashboards → explore → you inject `leaf1` `ethernet-1/1` (`python3 local/scripts/ssm-workshop-inject-fault.py start`) → they hunt → Infinity. Optional campus vendors (`fw-01`, `core-*`, `dist-*`, `access-*`, `tags_snmp_group=campus`) stay off the hunt dashboards. Building 4 / Check Point / EdgeConnect names stay Infinity mocks. See [`local/workshop/README.md`](local/workshop/README.md).

**Open work (start here next session):** Topology-exporter **restart hole is fixed and deployed** (LD-13 hold: empty Alloy ingest must not overwrite `snapshot.json`; colocated glue on `/var/lib/topology-exporter`). Do not re-litigate that. Next product slice: (1) Alloy flow `devicejoin` so catalog names (`client1`) beat EC2 PTRs, (2) `leaf1` missing gnmic LLDP + occasional BGP session drop, (3) one-page design-partner slice (what v1 is / is not). Still open: NLB TCP `:8080` + NLB traceroute from the colocated VPC (`.cursor/rules/open-aws-synthetic-probe.mdc`); Host Monitoring **Host** ↔ conversation **Endpoint** join when that preview is on this stack (`local/fixtures/conversation-kg/README.md` § Open). Lab stays **AWS colocated**, ktranslate + Alloy parallel — do not start the laptop Clos.
...
| **AWS colocated** | [`terraform/colocated-network-lab/`](terraform/colocated-network-lab/) — ContainerLab fabric + **k3s** (`make -C local colocated-lab-up`). Fresh EC2: `userdata.sh.tpl` → `colocated-host-deps.sh` → `colocated-ec2-bootstrap.sh` → systemd `network-o11y-fabric` / `network-o11y-telemetry`. Recovery: `colocated-bootstrap-recover.sh`. |
...
| AWS from **Windows** `aws`; Docker/terraform from **WSL** docker | Call `aws.exe` from WSL (`Exec format error`); assume Windows Docker Desktop is up; **auto-open SSO/OAuth** (personal browser has no work AWS/Grafana — prompt in chat, then `--use-device-code`) |
...
| NetBox OSS + Diode + Orb (**AWS colocated**) | `make -C local netbox-colocated` then SoT overlay `python3 local/scripts/netbox-populate.py --profile colocated` and `python3 local/scripts/orb-deploy-colocated.py --dry-diode` so Diode does not recreate `network-lab` ghosts / unused IF-MIB ports — UI: http://network-o11y-netbox-ui-383fcaf9622d867c.elb.us-east-1.amazonaws.com:8000/ (login `admin` / `NETBOX_ADMIN_PASSWORD` in `local/.env`; rotate with `python3 local/scripts/rotate-netbox-admin-password.py`) — [`local/netbox/README.md`](local/netbox/README.md) |
| Orb only (dry-run / re-push) | `make -C local orb-colocated` — [`local/orb/README.md`](local/orb/README.md) |
| Alloy-native SNMP (opt-in, parallel to ktranslate) | Laptop light: `make -C local alloy-snmp-min`. Full Clos: `LAB_ALLOY_SNMP=1` + `make alloy-snmp-up`. Staggered scrapes: **hot** = alerting/troubleshooting (60s: octets/oper), **cold** = names/MAC + **packet counters** (ucast/mcast/bcast) + **errors** + **discards** + IP inventory (`ip_addr`), **topology** = optional LLDP experiments. Device identity is one `snmp_device_info` inlined on the fingerprint module (`device_base` for unknown sysObjectID) — not a sibling `system_mib` scrape. Fingerprinters and `snmp-network.yml` are one catalog (same convert / same image); unknown `module=` names walk empty or can panic. Each scrape tier is optional (`LAB_ALLOY_SNMP_TIERS=hot` is the minimum useful walk; default `hot,cold`; add `topology` for LLDP/BGP-class). Portable CLI + library: [Mesverrum/snmp-sd](https://github.com/Mesverrum/snmp-sd) (`v0.1.0`). Every vendor pack is split the same way: `{name}` hot (identity/CPU/mem), `{name}_sensors`/`{name}_ext` cold, `{name}_topo` topology. Nokia is `if_mib,nokia_srlinux` / `if_mib_meta,ip_addr,nokia_srlinux_sensors` / `nokia_srlinux_topo`. Live colocated `/24` sweep (2026-09-08) found five SRL; invented `nokia_srlinux_hot` is gone; the binary must share L2/L3 with the CIDR (`docker --network clab`) and `--snmp-config` must define the group auth names. BGP neighbor drop/flap is traps/syslog, not a poll interval — [`docs/alloy-network-fork.md`](docs/alloy-network-fork.md). **Traps as logs:** `LAB_ALLOY_SNMPTRAP=1` + `make alloy-snmptrap-up` (`otelcol.receiver.snmptrap`). **Syslog as logs:** `LAB_ALLOY_SYSLOG=1` + `make alloy-syslog-up` (`otelcol.receiver.syslog`, `protocol = "none"`). Both join `device_name` from `snmp-targets.yml`. **Fleet:** `make -C local alloy-fleet-up` enrolls remotecfg (`GC_FM_URL` auto-detected); pipeline `network_o11y_alloy` upsert needs `fleet-management:write` on `GC_FM_TOKEN` (OTLP key is read-only on this stack). River sets CIDRs and auth names; `snmp_config` / `config_file` / `mib_paths` default to the image library. |
| Alloy cutover (colocated k3s) | `LAB_KTRANSLATE=0` + `make -C local alloy-cutover-colocated` — scales ktranslate to 0, enrolls Alloy `remotecfg`, upserts Fleet pipeline `network_o11y_alloy` (SNMP + traps/syslog + netflow; `auths = env("SNMP_AUTHS")`), stubs those from the k3s ConfigMap, and retargets device exporters to Alloy (`:1620` / `:1514` / `:6344` / `:2055`). Local ConfigMap keeps OTLP receive + export. Laptop `make up` still defaults to ktranslate. |
| ktranslate + Alloy fork (colocated) | `LAB_KTRANSLATE=1` + `python3 local/scripts/ssm-alloy-ktranslate-parallel.py` — scales ktranslate back to 1, turns remotecfg off so Fleet cannot bind `:1620`/`:1514`. Devices **dual-write**: ktranslate `:1620`/`:1621`/`:1622`/`:1514`/`:9995`/`:6343` and Alloy `:11620`/`:1515`/`:2055`/`:6344` plus Alloy SNMP scrape. Dual-ship `GC_OTLP_*_2` still fans ktranslate out to networko11ydev. |
| Alloy-native flow (opt-in, parallel to ktranslate) | `LAB_ALLOY_NETFLOW=1` + `make -C local alloy-netflow-up` — contrib `otelcol.receiver.netflow` on `:2055` / sFlow `:6344` (does **not** steal ktranslate `:9995`/`:6343`). Optional `targets` join `flow.sampler_address` → `device_name` from `device-join.yml` (SNMP catalog + fabric clients). Reverse-DNS (`udp_host_cache_size`, same LRU as `loki.source.syslog`) stamps `src_host` / `dst_host`; catalog name on PTR miss. **Metrics (default):** `otelcol.connector.signaltometrics` → `network.io.by_flow` (`integration=alloy-netflow`); PromQL is **`rate()`**, not ktranslate `* 8 / 60`. **Logs:** `LAB_ALLOY_NETFLOW_LOGS=1` punts decoded records onto the existing OTLP logs export (experimental; Loki is not the scale path). Needs `ALLOY_NETWORK_FROM_SOURCE=1` image. |
| Portable ktranslate on Kubernetes (not this Clos lab) | In [KtransToGrafana](https://github.com/Mesverrum/KtransToGrafana): `make k8s-up`. Same groups → generate → discover. Read `k8s/LIMITATIONS.md` first (stable collector IP, PVC device state, replicas=1, no active-active HA). This repo’s `k8s/ktranslate-golden` stays the colocated Clos translator. |
| Alloy SNMP dashboard | `make -C local alloy-snmp-dash` (UID `alloy-snmp-device-details`, curated `snmp_*` names) |
| Alloy SNMP composites | Grafana-managed recording rules (colon names): `make -C local alloy-snmp-recording-rules`. 1m group = memory/octets/util; 5m cold group = errors/s + error %. Packet rates are not recorded. Catalog: [`docs/alloy-network-fork.md`](docs/alloy-network-fork.md) § Computed metrics |
| Conversation / KG | Grafana-managed 4-tuple conversation rules + Endpoint/NetworkDevice/**Interface** entities on **marcnetterfield1**: `make -C local conversation-kg`. Alloy-only: `increase(alloy_network_io_by_flow_bytes)` + `snmp_CPU` / `snmp_if*`. Drops ephemeral src ports and collector noise. Flow `device_name` is the exporter (client), not the SRL hop — attach is a lab catalog (`client1→leaf1`, …). Interfaces are role-bearing (leaf `ethernet-1/1`, WAN ifAlias, fabric catalog ports) plus currently faulted physical ethernet — not every SNMP iface. Interface CONNECTS_TO Interface from the fabric/WAN catalog **and** live `network_topology_edge_info` when both ports are named (`Eth-*` → `ethernet-*`). KG Interface lookup aliases `src_interface`/`dst_interface`; `peer_interface` is a PROPERTY_MATCH fallback. Endpoint conversations stay ROUTES. Lab clients are **Endpoint** (not Host) so Grafana Host Monitoring preview can own Host later. |
| NetworkDevice Insights | Grafana-managed measurements + KG-labeled alerts: `make -C local network-device-insights`. Alloy `snmp_CPU` / `device:snmp_MemoryUtilization:percent` / `up{job="alloy-snmp"}` / `snmp_ifOperStatus==2`. Leaves the ktranslate ops group in place until cutover. |
| Alloy network alerts | Parallel Grafana group `Network Lab / alloy`: `make -C local alloy-network-alerts` (or `python3 local/scripts/provision-network-alerts.py --alloy`). Leaves the ktranslate group in place. |
...
- **NetBox Cloud (optional):** `scripts/netbox-populate.py` + `update-netbox-mgmt-ips.py` when `DISCOVERY_SOURCE=netbox` in `groups/srl-hq.env` (`groups/srl-hq.env.netbox.sample`). Colocated SoT overlay (sites, cables, WAN circuits, clients): `python3 local/scripts/netbox-populate.py --profile colocated`. Default bring-up uses **CIDR** discovery (`groups/srl-hq.env.sample`). See [`local/netbox/README.md`](local/netbox/README.md).
- **NetBox OSS + Diode + Orb (prefer AWS):** `make -C local netbox-colocated` on colocated EC2 — UI via `make netbox-ui-tunnel` → http://127.0.0.1:8000/ or the public NLB `:8000`. **SoT is populate** (`python3 local/scripts/netbox-populate.py --profile colocated`: 9 devices, role-bearing ifaces, cables, WAN circuits). Orb discovery stays dry-run after tidy (`python3 local/scripts/orb-deploy-colocated.py --dry-diode`) — live Diode reclutters same-name copies at site `network-lab` plus unused IF-MIB ports. Grafana Cloud reads NetBox via Infinity `netbox-api` (dashboards 20/22/24/25). Inventory audit history stays in NetBox changelog/journal. Orb OTLP → Alloy. Does **not** replace ktranslate SNMP. Docs: [`local/netbox/README.md`](local/netbox/README.md), [`local/orb/README.md`](local/orb/README.md).
...
- **SNMP profiles:** bundled in the ktranslate image from [kentik/snmp-profiles](https://github.com/kentik/snmp-profiles). Discovery matches `sysObjectID` → `mib_profile` automatically (e.g. Nokia SR Linux → `nokia-srlinux.yml`). Optional fork: `PROFILE_GIT_URL` in `local/.env` (`make generate` → poller/discovery `profile_git_url`). Private: `KT_GIT_ACCESS_*`. Missing platform? [Profile tutorial](https://github.com/kentik/ktranslate/wiki/Tutorial:-Writing-a-custom-yaml-file-for-SNMP) → PR upstream — do not bind-mount local profile overrides in normal bring-up.
...
- **Topology graph:** gnmic LLDP and Alloy SNMP topology walks fork to **topology-exporter** `/v1/metrics` → `graph.Reconcile`. Alloy scrapes exporter `:9100` so `network_topology_edge_info` (`inference=alloy_otlp`, `evidence=gnmi_lldp` or `nokia_bgp_peer`) reaches Grafana Cloud. Alloy does not forge edges in River. Optional native SNMP walkers stay behind `skip_native_snmp`. Process restart keeps last-known edges from `snapshot.json` (`graph_stale=1`) until Alloy ingest has samples — empty buffer must not overwrite the snapshot.
...
| Stream | PromQL / check |
|--------|----------------|
| SNMP | `count by (device_name) (kentik_snmp_CPU)` → spine1, leaf1, leaf2; `count({__name__=~"kentik_snmp.*"})` for volume. **Not** `kentik_snmp_DeviceMetrics`. ktranslate stamps **`tags_snmp_group`**. Alloy stamps **`snmp_group`** = discovery job name (`hq` on laptop; `hq` / `branch1` / `branch2` on colocated — one `discovery.snmp` group per fake site). Filter dashboards with `snmp_group=~"$snmp_group"`. Deeper inventory (role, CMDB site) → NetBox, not a second Prometheus label. Discovery health is `discovery_snmp_*{job="alloy"}` (not `integrations/alloy`). |
...
**Alloy network fork (networko11ydev folder [`fb9d2s`](https://networko11ydev.grafana.net/dashboards/f/fb9d2s/alloy-network-fork)):** A0–A4, queries on `snmp_*` / `alloy_network_io_by_flow_bytes` / Loki `{service_name="alloy-snmptrap"|"alloy-syslog"}`. Recreate: `python3 local/scripts/clone-alloy-folder-networko11ydev.py`. Recording rules: `python3 local/scripts/provision-alloy-snmp-recording-rules.py --stack2 --folder fb9d2s`.

| # | UID | Title |
|---|-----|-------|
| A0 | `alloy-architecture` | Alloy Architecture |
| A1 | `alloy-health` | Alloy Health (`rebuild-alloy-health.py` — scrape/flow/events, not CHF) |
| A2 | `alloy-flow-summary` | Alloy Flow Summary |
| A3 | `alloy-device-summary` | Alloy Device Summary |
| A4 | `alloy-device-details` | Alloy Device Details |

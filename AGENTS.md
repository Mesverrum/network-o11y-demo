...
**Open work (start here next session):** NLB TCP `:8080` and NLB traceroute still fail from the colocated VPC — see `.cursor/rules/open-aws-synthetic-probe.mdc`. Laptop SM traceroute was already removed. Details: [`local/synthetic-monitoring/README.md`](local/synthetic-monitoring/README.md). When Grafana Host Monitoring is on this stack, join native Host entities to conversation **Endpoint**s — do not mint a lab `Host` type; tracker: [`local/fixtures/conversation-kg/README.md`](local/fixtures/conversation-kg/README.md) § Open.

...
**Open work (start here next session):** NLB TCP `:8080` and NLB traceroute still fail from the colocated VPC — see `.cursor/rules/open-aws-synthetic-probe.mdc`. Laptop SM traceroute was already removed. Details: [`local/synthetic-monitoring/README.md`](local/synthetic-monitoring/README.md).
...
| Conversation / KG | Grafana-managed 4-tuple conversation rules + Host/NetworkDevice/**Interface** entities on **marcnetterfield1**: `make -C local conversation-kg`. Drops ephemeral src ports and collector noise. Flow `device_name` is the exporter (client), not the SRL hop — attach is a lab catalog (`client1→leaf1`, …). Interfaces are role-bearing (leaf `ethernet-1/1`, WAN ifAlias, fabric catalog ports) plus currently faulted physical ethernet — not every SNMP iface. Interface ROUTES Interface from the fabric/WAN catalog **and** live `network_topology_edge_info` when both ports are named (`Eth-*` → `ethernet-*`). KG Interface lookup aliases `src_interface`/`dst_interface`; `peer_interface` is a PROPERTY_MATCH fallback. |
| NetworkDevice Insights | Grafana-managed measurements + KG-labeled alerts: `make -C local network-device-insights`. CPU/mem/SNMP-unhealthy/`ethernet-1/1` oper-down on NetworkDevice; Interface oper-down on the sparse Interface entities. Leaves the ktranslate ops group in place. |

...
| ktranslate + Alloy fork (colocated) | `LAB_KTRANSLATE=1` + `python3 local/scripts/ssm-alloy-ktranslate-parallel.py` — scales ktranslate back to 1, turns remotecfg off so Fleet cannot bind `:1620`/`:1514`, keeps Alloy SNMP scrape + netflow `:2055`/sFlow `:6344`, and retargets traps/syslog/softflowd to ktranslate. Dual-ship `GC_OTLP_*_2` still fans ktranslate out to networko11ydev. |

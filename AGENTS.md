...
- **Topology graph:** gnmic LLDP and Alloy SNMP topology walks fork to **topology-exporter** `/v1/metrics` → `graph.Reconcile` → `network_topology_edge_info` (`inference=alloy_otlp`, `evidence=gnmi_lldp` or `nokia_bgp_peer`). Alloy does not forge edges in River. Optional native SNMP walkers stay behind `skip_native_snmp`.

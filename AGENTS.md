...
**Current lab (until the operator says otherwise):** **AWS colocated only.** Do not start the laptop Clos, Compose Alloy, or `make alloy-snmp-min` / `make alloy-fleet-up` on the laptop. Collector work is Fleet `network_o11y_alloy` + k3s Alloy on the colocated host (`LAB_KTRANSLATE=0`).

#!/usr/bin/env python3
"""Sync host check script and run Alloy cutover smoke checks on colocated EC2."""
...
SCRIPT = REPO / "local" / "scripts" / "_check-alloy-cutover-host.sh"
...
def send(commands: list[str], timeout: int, label: str) -> None:
    ...
    for _ in range(timeout // 5 + 10):
        ...
        if status in {"Success", "Failed", "Cancelled", "TimedOut"}:
            print(stdout[-6000:] if stdout else "(no stdout)")
            if stderr:
...
def main() -> None:
    dest = "/opt/network-o11y-demo/local/scripts/_check-alloy-cutover-host.sh"

...
COLOCATED_COMPOSE_OUT="${REPO_ROOT}/compose-colocated.generated.yaml"
...
# GENERATED by scripts/generate-groups.sh — do not edit by hand.
# Used when COLLECTOR_RUNTIME=k3s: SNMP discovery on host network, OTLP → k3s Alloy.
#   COLLECTOR_RUNTIME=k3s docker compose -f compose-base.yaml ... -f compose-colocated.generated.yaml \\
#     --profile discovery run --rm discover_<group>
services:
EOF
...
# ---------- Device catalog for flow / sFlow / syslog receivers ----------
...

{
  ...
  echo "# Enrichment-only catalog: flow, sFlow, and syslog match device_ip → device_name"
...

{
  ...
  echo "services:"
  for svc in ktranslate_flow ktranslate_sflow ktranslate_syslog; do
    echo "  ${svc}:"
    echo "    volumes:"
    echo "      - ./config/catalog.yaml:/snmp.yaml:ro"
    for group in "${CATALOG_GROUPS[@]}"; do
      echo "      - ./state/devices-${group}.yaml:/state/devices-${group}.yaml:ro"
    done
  done
} > "${CATALOG_COMPOSE_OUT}"
...
echo
echo "wrote $(ls "${CONFIG_DIR}"/discovery-*.yaml 2>/dev/null | wc -l) discovery configs, $(ls "${CONFIG_DIR}"/poller-*.yaml 2>/dev/null | wc -l) poller configs, ${CATALOG_OUT}, ${CATALOG_COMPOSE_OUT}, ${COMPOSE_OUT}, ${COLOCATED_COMPOSE_OUT}, and compose-host.generated.env"

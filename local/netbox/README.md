...
SNMP discovery works **without NetBox** using `DISCOVERY_SOURCE=cidr` in `groups/srl-hq.env`
(the default in `groups/srl-hq.env.sample`). Use this folder when you want inventory-driven
...
```bash
cp groups/srl.env.netbox.sample groups/srl-hq.env
```
...
make up                # or: make discover GROUP=srl-hq after fabric is up
```

After clab recreate (mgmt IP drift): `make netbox-sync-mgmt` then `make discover GROUP=srl-hq`.

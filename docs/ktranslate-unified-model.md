...
3. **SNMP uses discover → poller** on the laptop lab (`make discover`); EKS uses a static device list in `k8s/telemetry/ktranslate-config.yaml`. The poller only walks `global.mibs_enabled`. `make generate` / `make discover` unions each device's `discovered_mibs` onto that list (seed is `IF-MIB` plus lab `TIMETRA-*`). Pin with `MIBS_ENABLED=` or `ADD_DISCOVERED_MIBS=0` on the group file.
...
3. Run discovery — **MIBs and SNMP profiles are usually handled automatically.** ktranslate ships with [Kentik snmp-profiles](https://github.com/kentik/snmp-profiles); discovery matches each device's `sysObjectID` to a profile. Optional fork: `PROFILE_GIT_URL` in `local/.env` then `make generate` (ktranslate `profile_git_url`). If your platform is missing, follow the [ktranslate SNMP profile tutorial](https://github.com/kentik/ktranslate/wiki/Tutorial:-Writing-a-custom-yaml-file-for-SNMP) and open a PR to [kentik/snmp-profiles](https://github.com/kentik/snmp-profiles) rather than maintaining a local override.
